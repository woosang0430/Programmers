...
def explore_around(maps, x, y):
    ...
    for i in range(4):
        ny = y + dy[i]
        nx = x + dx[i]
        if ny in range(Y) and nx in range(X) and maps[ny][nx] == 0:
            maps[ny][nx] = 2
            dq.append((ny, nx))
...
for combine in list(combinations(zero_idx, 3)):
    ...
    for y, x in combine:
        maps_copy[y][x] = 1
        # 모든 경우의 벽을 세운다.
    # 초기 virus의 위치 정보를 넘겨주고 DFS
    stack = copy.deepcopy(virus_locations)
    while stack:
        y, x = stack.pop()
        explore_around(maps_copy, x, y)
    # 0의 갯수가 많은 것을 남기기
